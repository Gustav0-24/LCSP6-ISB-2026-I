...
El ECG es una señal bioeléctrica débil, muy suceptible al ruido de diversas fuentes, tanto internas como externas (al cuerpo del paciente). Los movimientos, las señales eléctricas de otros músculos, la mala conexión de electrodos pueden generar una lectura de ECG borrosa, por lo que es necesario aplicar un filtrado para mitigar esa interferencia. Para este laboratorio se aplicaron los siguientes filtros:
- **Pasa-banda (0.5 Hz-40 Hz):** Filtrado suave para entornos ruidosos. Se usa principalmente para detectar la frecuencia cardiaca.
...
Las señales de ECG obtenidas con el sistema BITalino mostraron diferencias evidentes entre las condiciones fisiológicas analizadas: reposo basal, conservación de la respiración, descanso después y ejercicio físico. En todos los casos de adquisición, se detectó una señal cruda con interferencia de línea eléctrica, variaciones en la línea base y ruido de alta frecuencia. Esto fue especialmente evidente durante las maniobras respiratorias y la actividad física. Después, se logró un incremento notable en la calidad de la señal al aplicar el filtro pasabanda de 0.5-40 Hz y el filtro notch de 60 Hz, lo cual mantuvo las componentes principales del ECG y suprimió una gran cantidad de interferencias externas.

La señal filtrada presentó una morfología cardíaca más estable y cíclica en el estado de reposo basal. La onda P, el complejo QRS y la onda T pudieron ser identificados con más claridad, lo que señala que el procesamiento utilizado logró preservar la información fisiológica importante del electrocardiograma. El complejo QRS, que representa la despolarización del ventrículo y normalmente dura menos de 0.12 segundos, y la onda P, que corresponde a la despolarización auricular [1], son mencionados en la literatura. Además, el espectro FFT de la señal filtrada mostró una disminución significativa de elementos de alta frecuencia, lo que corroboró la eficacia del filtrado digital que se implementó.

Las señales ECG mostraron cambios en la regularidad temporal de los complejos cardíacos durante las pruebas de mantenimiento respiratorio. En múltiples registros se notaron ligeras diferencias en la amplitud y separación entre complejos QRS, que probablemente están vinculadas a modificaciones fisiológicas en la modulación autonómica y en la dinámica de la respiración. Asimismo, las señales crudas evidenciaron una mayor sensibilidad al ruido y a la alteración de la línea base, a causa de movimientos involuntarios y tensión en los músculos. No obstante, después del procesamiento digital, se logró restablecer la morfología cardíaca de manera apropiada. Esto evidencia lo importante que es el preprocesamiento en los sistemas biomédicos de adquisición de señales, porque una interpretación errónea puede dar lugar a diagnósticos equivocados [2].

Los niveles más altos de contaminación acústica se registraron en las grabaciones relacionadas con la actividad física. Las gráficas evidenciaron un aumento de artefactos por actividad muscular y movimiento, lo que produce componentes más amplios en el dominio de la frecuencia. Sin embargo, tras aplicar los filtros notch y pasabanda, los complejos QRS mantuvieron su distinción, lo que posibilitó el análisis de la actividad del corazón. Asimismo, se notó un aumento de la frecuencia cardíaca, que se tradujo en una reducción del tiempo entre complejos QRS sucesivos. Este fenómeno es fisiológicamente esperado durante el ejercicio por la activación simpática y porque la demanda metabólica aumenta.

Los espectros FFT en dB y en escala lineal hicieron posible comprobar de manera experimental el comportamiento de las señales ECG a nivel de frecuencia. Los componentes que se distribuyeron en un extenso rango de frecuencias, incluyendo picos asociados a interferencia eléctrica cerca de 60 Hz, fueron detectados en las señales crudas. Después de la filtración, esas componentes se redujeron considerablemente, concentrando la mayor energía en las frecuencias bajas relacionadas con el funcionamiento cardíaco normal. Este comportamiento es consistente con lo que se ha descrito para las señales ECG clínicas, en las que se observa que la mayoría de la información útil está por debajo de los 40 Hz.

La correcta interpretación del ECG tiene una gran importancia clínica porque posibilita la detección de trastornos de conducción, arritmias, hipertrofias y modificaciones cardíacas que pueden ser graves [2]. Por lo tanto, para prevenir fallos de interpretación, es fundamental adquirir y procesar las señales adecuadamente. En este marco, los resultados logrados evidencian que el empleo de filtros digitales optimiza significativamente la calidad diagnóstica del ECG, lo cual ayuda a reconocer intervalos y ondas significativas.

Adicionalmente, los resultados evidencian cómo el reconocimiento visual de patrones ECG depende de la calidad de la señal procesada. Estudios recientes señalan que la interpretación electrocardiográfica requiere habilidades de reconocimiento de patrones y entrenamiento continuo, ya que incluso profesionales de salud pueden cometer errores ante señales ruidosas o complejas [3]. La mejora obtenida tras el filtrado favorece precisamente este reconocimiento morfológico, aumentando la claridad de los complejos cardíacos y reduciendo posibles ambigüedades.

Finalmente, este laboratorio permitió comprobar experimentalmente la utilidad del procesamiento digital aplicado a señales biomédicas. La implementación de filtros Butterworth pasabanda y filtros notch permitió obtener señales ECG más limpias y fisiológicamente interpretables en diferentes condiciones experimentales. Estos resultados evidencian la importancia del acondicionamiento digital de señales en aplicaciones biomédicas modernas, especialmente en sistemas portátiles de monitoreo cardíaco y plataformas de adquisición en tiempo real.
